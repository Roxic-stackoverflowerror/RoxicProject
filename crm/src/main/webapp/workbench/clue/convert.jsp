    ...
    <script type="text/javascript">
        $(function () {
            ...
            $("#convertBtn").click(function () {
                /*
                * 提交请求到后台，执行线索转换的操作，应该发出传统请求，请求结束后，最终响应线索列表页
                *
                * 根据为客户创建交易的复选框有没有选择，判断是否需要创建交易
                * */
                if($("#isCreateTransaction").prop("checked")){
                    //需要创建交易
                    //window.location.href = "workbench/clue/convert.do?clueId=<%=clueId%>&money=xxx&execpedDate=xxx&name=xxx&stage=xxx&activityId=xxx";
                    //以上传递参数方式很麻烦，而且表单一旦扩充，挂在的参数有可能超出浏览器上限

                    //提交交易表单形式来发出本次请求
                    //优点：提交表单的参数不用手动去填，提交表单可以提交post
                    $("#tranForm").submit();
                    //$("#tranForm")[0].reset();
                }else{
                    //不需要创建交易
...
<div id="title" class="page-header" style="position: relative; left: 20px;">
    <h4>转换线索
        <small><%=fullname%>&nbsp;<%=appellation%>-<%=company%></small>
    </h4>
</div>
...
<div id="create-transaction2"
    ...
    <form id="tranForm" action="workbench/clue/convert.do" method="post">
        ...
        <input type="hidden" name="clueId" value="<%=clueId%>">
        ...
        <div class="form-group" style="width: 400px;position: relative; left: 20px;">
            ...
            <select id="stage" class="form-control" name="stage">
                <option></option>
                <c:forEach items="${stage}" var="roxic">
                    <option value="${roxic.value}">${roxic.text}</option>
                </c:forEach>
            </select>
        </div>
        <div class="form-group" style="width: 400px;position: relative; left: 20px;">
...
<div id="operation" style="position: relative; left: 40px; height: 35px; top: 100px;">
    ...
    &nbsp;&nbsp;&nbsp;&nbsp;
    <input class="btn btn-default" type="button" value="取消" onclick="window.location.href='workbench/clue/detail.do?id=<%=clueId%>'">
</div>
</body>
</html>